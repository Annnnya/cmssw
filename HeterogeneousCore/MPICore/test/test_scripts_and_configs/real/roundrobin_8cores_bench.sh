...
BASE_DIR="../../test_results/3remotes-roundrobin"
mkdir -p "$BASE_DIR"
...
for combo in "${thread_stream_combos[@]}"; do
    ...
    for i in $(seq 1 $runs); do
        ...
        -np 1 numactl --physcpubind=24-"${end_core_other1}" cmsRun "$script_remote" \
        : -np 1 numactl --physcpubind=16-"${end_core_other2}" cmsRun "$script_remote" \
        : -np 1 numactl --physcpubind=8-"${end_core_other3}" cmsRun "$script_remote" \
