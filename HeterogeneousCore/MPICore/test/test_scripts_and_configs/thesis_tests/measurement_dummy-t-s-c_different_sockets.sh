...
BASE_DIR="../../test_results_thesis/dummy/openmpi/simple_async/different_sockets"
mkdir -p "$BASE_DIR"
...
for message_size in "${message_sizes[@]}"; do
    ...
    for combo in "${thread_stream_combos[@]}"; do
        ...
        for i in $(seq 1 $runs); do
            ...
            mpirun -np 1 -bind-to none numactl --physcpubind=64-"${end_core_other}" cmsRun "$script_remote" \
...
echo "All dummy local-remote tests completed!"
