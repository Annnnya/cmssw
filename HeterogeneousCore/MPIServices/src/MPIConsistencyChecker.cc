...
void MPIConsistencyChecker::recordMPIModuleInfo_(bool is_sender,
  ...
  for (auto const& existing_module : modules_info_) {
    if (existing_module.module_label == module_label) {
      return;
    }
  }
...
void MPIConsistencyChecker::getSerializedMPIModuleInfo(
  ...
  if (!cls) {
    throw cms::Exception("MPIConsistencyChecker")
        << "Failed to get TClass for std::vector<MPIModuleInfo>";
  }
...
void MPIConsistencyChecker::deserializeMPIModuleInfo(
...
void MPIConsistencyChecker::reconstructMPIPaths() {
  std::call_once(paths_reconstructed_flag_, [&]() {
    ...
    for (size_t i = 0; i < modules_info_.size(); ++i) {
      if (!module_assigned[i]) {
        throw cms::Exception("MPIConsistencyChecker") << "Could not assign MPI module " << modules_info_[i].module_label << " to any MPI path";
      }
    }
  });
}
...
void MPIConsistencyChecker::compareMPIModules(std::vector<MPIModuleInfo> const& other,
  ...
  std::lock_guard<std::mutex> lock(modules_info_mutex_);
  auto const& local_modules = modules_info_;
  if (local_modules.size() != other.size()) {
    throw cms::Exception("MPIConsistencyChecker") << "Mismatch in number of MPI modules between process " << this_process_name
                                                  << " and process " << other_process_name << ": " << local_modules.size() << " vs " << other.size();
  }
  for (auto const& local_module : local_modules) {
    ...
    if (it == other.end()) {
      throw cms::Exception("MPIConsistencyChecker") << "No matching sender/receiver found in process " << other_process_name << " for MPI module instance " << local_module.instance << " in process " << this_process_name;
    }
    if (local_module.product_types.size() != it->product_types.size()) {
      throw cms::Exception("MPIConsistencyChecker") << "Mismatch in number of product types between sender module "
                                                    << local_module.instance << " in process " << this_process_name
                                                    << " and receiver module " << it->instance << " in process "
                                                    << other_process_name << ": " << local_module.product_types.size()
    ...
    for (size_t i = 0; i < local_module.product_types.size(); ++i) {
      if (local_module.product_types[i] != it->product_types[i]) {
        throw cms::Exception("MPIConsistencyChecker") << "Mismatch in product type at index " << i
                                                      << " between sender module " << local_module.instance
                                                      << " in process " << this_process_name << " ("
                                                      << local_module.product_types[i] << ") and receiver module "
                                                      << it->instance << " in process " << other_process_name << " ("
                                                      << it->product_types[i] << ")";
      }
    }
  }
